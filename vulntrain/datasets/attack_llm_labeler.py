...
def slugify_model(backend: str, model: str) -> str:
    """`ollama`, `qwen3.6:35b` -> `ollama-qwen3.6-35b` (Hub-repo-name safe)."""
    slug = re.sub(r"[^0-9A-Za-z._-]+", "-", f"{backend}-{model}")
    return slug.strip("-")
...
def build_card_body(
    ...
    """Human-readable dataset-card section recording which model produced the
    labels (appended to the auto-generated card so several test runs stay
    distinguishable)."""
    lines = [
        "## LLM-labeled expansion — provenance",
        "",
        f"⚠️ These labels are **machine-generated by `{backend}/{model}`**, not "
        "analyst-curated. They follow the MITRE CTID "
        '"Mapping ATT&CK to CVE for Impact" methodology as an expansion of the '
        f"curated gold dataset "
        f"[{gold_dataset}](https://huggingface.co/datasets/{gold_dataset}).",
        "",
        f"- **Labeling model**: `{backend}/{model}`",
        f"- **CVEs**: {n_rows}",
        f"- **ATT&CK version**: {attack_version}",
        '- **`label_sources`**: `["llm"]` on every row; the `llm_model` column '
        "records the exact model per row",
    ]
    if agreement_note:
        lines.append(f"- **Validation agreement vs gold set**: {agreement_note}")
    else:
        lines.append(
            "- **Validation agreement vs gold set**: run the labeler in "
            "`--mode validate` with the same model and record the score here "
            "before relying on these labels"
        )
    lines += [
        "",
        "Intended for training augmentation alongside the analyst-curated gold "
        "set — keep the `label_sources` column so gold-only can always be "
        "recovered. Generated with "
...
def push_dataset_card(repo_id: str, card_body: str) -> None:
    """Append the provenance section to the auto-generated dataset card,
    preserving its YAML front matter and dataset_info."""
    from huggingface_hub import HfApi

    api = HfApi()
    try:
        existing = Path(
            api.hf_hub_download(repo_id, "README.md", repo_type="dataset")
        ).read_text()
    except Exception:
        existing = ""
    card = f"{existing.rstrip()}\n\n{card_body}" if existing else card_body
    api.upload_file(
        path_or_fileobj=card.encode("utf-8"),
        path_in_repo="README.md",
        repo_id=repo_id,
        repo_type="dataset",
        commit_message="Record labeling model in dataset card",
    )
...
def run_expand(args: argparse.Namespace, catalog: dict[str, str]) -> None:
    ...
    if args.output_dir:
        ...
        Path(args.output_dir, "README.md").write_text(card_body)
        logger.info(f"Saved to {args.output_dir}")

    if args.push:
        # Give each model its own repo so multiple test runs don't overwrite
        # each other; opt out with --no-model-suffix.
        repo_id = args.repo_id
        if not args.no_model_suffix:
            repo_id = f"{repo_id}-{slugify_model(args.backend, args.model)}"
        dataset.push_to_hub(
            repo_id,
            commit_message=(
        ...
        push_dataset_card(repo_id, card_body)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Base Hub repo ID for the expanded dataset (expand mode). The "
        "backend/model slug is appended unless --no-model-suffix is set.",
    )
    parser.add_argument(
        "--no-model-suffix",
        action="store_true",
        help="Push to --repo-id verbatim instead of appending the model slug.",
    )
    parser.add_argument(
        "--agreement-note",
        default="",
        help="Validation agreement to record on the dataset card, e.g. "
        "'f1_micro 0.61 on the test split'.",
    )
    parser.add_argument(
        "--push",
        action="store_true",
